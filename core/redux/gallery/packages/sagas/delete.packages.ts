...
export function *handleDeletePackage
  (action: ReturnType<typeof GalleryPackageActions.delete_request>) {
  ...
  try {
    ...
    if (response.meta.ok) {
      yield put(GalleryPackageActions.delete_success({
        meta: {
      ...
      yield put(GalleryPackageActions.delete_failed({ 
        server_status: 'error', 
        server_action: action.meta.do,
...
export function *galleryPackageDeleteSaga() {
  yield takeEvery(getType(GalleryPackageActions.delete_request), handleDeletePackage)
}